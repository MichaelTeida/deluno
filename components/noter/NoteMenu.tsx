    ...
    return (
        <div className="relative" ref={menuRef}>
            <button
                ...
                className={`glass w-10 h-10 flex items-center justify-center transition-all duration-200 rounded-full ${isOpen ? "text-zinc-700 dark:text-zinc-200 bg-white/30 dark:bg-white/10" : "text-zinc-400 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200"}`}
                data-variant="interactive"
                title="Options"
            >
                <svg className={`w-5 h-5 transition-transform duration-200 ${isOpen ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
