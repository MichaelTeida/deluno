    ...
    return createPortal(
        <div className="flex items-center gap-1">
            ...
            <button
                ...
                className={`glass w-8 h-8 md:w-9 md:h-9 flex items-center justify-center transition-all duration-200 ${note.isFavorite ? "text-amber-500 bg-amber-500/10" : "text-zinc-400 dark:text-zinc-400 hover:text-amber-500 dark:hover:text-amber-400"}`}
                data-variant="interactive"
                title={note.isFavorite ? "Remove from favorites" : "Add to favorites"}
            >
                <svg className={`w-4 h-4 md:w-5 md:h-5 transition-transform duration-200 ${note.isFavorite ? "scale-110" : ""}`} fill={note.isFavorite ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
